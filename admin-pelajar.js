...
async function renderPelajar(){
  ...
  $('#p-add').onclick=async()=>{const name=$('#p-nama').value.trim(),nokp=$('#p-nokp').value.trim().replace(/-/g,''),kelas=$('#p-kelas').value.trim();if(!name||!nokp){toast('Nama & NoKP wajib');return;}
  ...
  $('#p-templat').onclick=()=>download('templat_pelajar.csv','Nama,NoKP,Kelas,Semester\nMuhammad Haziq,050101071234,1A,1\nNur Aisyah,050202072222,1A,1');
...
async function importCSV(text){
  try{
    ...
    const iN=find(['nama','name']),iK=find(['nokp','nokad','ic','no']),iC=find(['kelas','class']),iS=find(['semester','sem']);
    if(iN<0||iK<0)throw new Error('Lajur perlu: Nama, NoKP');
    const batch=[];for(let r=1;r<rows.length;r++){const c=rows[r];const name=(c[iN]||'').trim(),nokp=(c[iK]||'').trim().replace(/-/g,''),kelas=(iC>=0?(c[iC]||'').trim():''),semester=parseInt(c[iS]||'0')||0;if(!name||!nokp)continue;batch.push({name,nokp,kelas,semester:semester||null});}
